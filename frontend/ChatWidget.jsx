...
export default function ChatWidget() {
  ...
  const send=async e=>{
    ...
    setInput('');setLoading(true);setUserCount(c=>c+1);
    try{
    ...
    }catch{setMessages(m=>[...m,{from:'bot',text:'Erreur serveur.'}]);}
    finally{setLoading(false);}
  };

  useEffect(()=>{if(userCount>=3&&!showLead) setShowLead(true);},[userCount,showLead]);

  const saveLead=e=>{e.preventDefault();setShowLead(false);};

  return (
    <div className="chat-widget" role="dialog" aria-label="Chat IA Widget">
      <div className="chat-header"><img src={logo} alt="" className="logo"/><span>{BOT_NAME}</span></div>
      ...
      {showLead&&<form className="lead-form" onSubmit={saveLead}>
